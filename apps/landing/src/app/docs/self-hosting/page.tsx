    ...
    >
      <div className="space-y-16">
        <p className="text-xl leading-relaxed text-foreground/80 font-medium">
          Replaysafe is built for the **Sovereign Homelab** and the **Next
          Generation of Autonomous Agents**. While enterprise teams use us for
          financial transaction safety, self-hosters use us to build more
          reliable AI agents and keep their automated lives from falling apart.
        </p>
        ...
        <section className="glass-panel border border-border/10 rounded-[3rem] p-12 bg-acid-lime/[0.02] space-y-8">
          ...
          <div className="grid gap-8 pt-4">
            <HomelabUseBox
              ...
              description="If a 'Lock Doors' or 'Turn Off Stove' command fails halfway through a Wi-Fi blink, Replaysafe ensures it reverts to a safe state."
            />
            <HomelabUseBox
        ...
        <section className="space-y-8">
          ...
          <p className="leading-relaxed text-muted-foreground">
            Replaysafe is 100% locally hostable. No telemetry, no external cloud
            dependencies, and 100% local data sovereignty.
          </p>
        ...
        <section className="space-y-8 pt-16 border-t border-border/5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <SovereignFeature
              ...
              desc="We never send your data to external servers."
            />
            <SovereignFeature
              icon={<Globe className="w-5 h-5" />}
              title="Offline Mode"
              desc="Fully functional in air-gapped networks."
            />
            <SovereignFeature
              icon={<Cpu className="w-5 h-5" />}
              title="ARM64 Support"
              desc="Optimized for Raspberry Pi 4/5."
            />
            <SovereignFeature
              icon={<Shield className="w-5 h-5" />}
              title="AGPL-3.0"
              desc="Committed to FOSS forever."
            />
          </div>
        </section>
      </div>
...
function HomelabUseBox({
  ...
  return (
    <div className="p-6 rounded-2xl bg-foreground/[0.03] border border-border/5">
      <h4 className="font-black uppercase tracking-widest text-xs text-acid-lime mb-2">
        {title}
      </h4>
...
function SovereignFeature({
  ...
  return (
    <div className="flex gap-4">
      ...
      <div>
        <h5 className="font-bold text-foreground text-sm uppercase tracking-widest mb-1">
          {title}
        </h5>
