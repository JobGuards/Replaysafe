...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // Log the action
      await auditService.logMonitorAction(
        "MONITOR_CREATE",
        monitorForAudit,
        req.user?.id,
      );
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const monitor = await monitorRepository.findById(
    ...
    try {
      ...
      if (validation.data.schedule || validation.data.scheduleType) {
        ...
        if (monitor) {
          ...
          const scheduleType =
            validation.data.scheduleType || monitor.scheduleType;
          const timezone = validation.data.timezone || monitor.timezone;

          // @ts-ignore
          updateData.nextExpectedAt = getNextExpectedDate(
            schedule,
            scheduleType,
            timezone,
          );
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      const monitor = await monitorRepository.delete(
        req.params.id as string,
        project.id,
      );
      if (!monitor) {
