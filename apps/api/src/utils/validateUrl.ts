...
export function validateWebhookUrl(urlString: string): URL {
  ...
  for (const pattern of BLOCKED_HOSTNAMES) {
    if (pattern.test(hostname)) {
      throw new Error(`Blocked hostname: ${hostname}. Internal/private hosts are not allowed.`)
    }
  }

  if (net.isIP(hostname) !== 0 && isPrivateIP(hostname)) {
    throw new Error(`Blocked IP: ${hostname}. Private/link-local IPs are not allowed.`)
  }
