...
async function checkMissedHeartbeats() {
  ...
  try {
    ...
    console.log(
      `[Worker] Found ${missedMonitors.length} monitors with missed heartbeats`,
    );
    ...
    for (const monitor of missedMonitors) {
      ...
      console.log(
        `[Worker] Monitor ${monitor.name} (${monitor.id}) marked as DOWN and incident created`,
      );
    }
  } catch (error: any) {
    if (isDbConnected) {
      console.error(
        "[Worker] Connection error - stopping log spam until restored:",
        error.message || error,
      );
      isDbConnected = false;
    }
...
export function startMissedHeartbeatWorker(intervalMs: number = 60000) {
  console.log(
    `[Worker] Starting missed heartbeat detection worker (Interval: ${intervalMs}ms)`,
  );
