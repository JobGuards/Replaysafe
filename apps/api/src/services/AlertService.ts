...
export class AlertService {
  ...
  private async sendToChannel(
    channel: any,
    data: AlertData & { durationText?: string },
  ) {
    const provider = this.providers[channel.type.toLowerCase()];

    if (!provider) {
      console.warn(
        `[AlertService] No provider found for channel type: ${channel.type}`,
      );
      return;
    }
    ...
    try {
      ...
      console.log(
        `[AlertService] Delivering to ${channel.type} (ID: ${channel.id})`,
      );
      ...
      console.log(
        `[AlertService] Config Keys: ${Object.keys(decryptedConfig || {}).join(", ")}`,
      );
      ...
      console.error(
        `[AlertService] Error sending alert to channel ${channel.id}:`,
        error,
      );
  ...
  private async getEnabledChannels(projectId: string) {
    ...
    return channels
      .map((c: any) => {
        try {
          const decryptedConfig =
            typeof c.config === "string" ? decryptJSON(c.config) : c.config;
          ...
          if (typeof decryptedConfig !== "object" || decryptedConfig === null) {
            console.warn(
              `[AlertService] Skipping channel ${c.id}: Configuration decryption failed.`,
            );
            return null;
          }
          ...
          console.warn(
            `[AlertService] Skipping channel ${c.id}: Decryption error.`,
          );
          return null;
        }
      })
