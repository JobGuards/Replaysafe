...
import {
  ReplayGuard,
  EffectTimeoutError,
} from "../../../../packages/guard-sdk/src/index.ts";
...
describe("guard.wrap() shim — mock call sequence regression", () => {
  ...
  it("re-throws operation errors (same as before)", async () => {
    ...
    await expect(
      guard.wrap("EMAIL", "user@test.com", {}, () =>
...
describe("ReplayGuard SDK (existing tests — must not regress)", () => {
  ...
  describe("Fingerprinting (Input Hashing)", () => {
    ...
    it("should support custom ignore keys", () => {
      ...
      const hash1 = (guard as any)._buildInputHash({
        amount: 100,
        customField: "foo",
      });
      const hash2 = (guard as any)._buildInputHash({
        amount: 100,
        customField: "bar",
      });
      expect(hash1).toBe(hash2);
  ...
  describe("Network Resilience", () => {
    it("should retry on fetch failure", async () => {
      ...
      const guard = new ReplayGuard({
        apiKey: "sk_test_123",
        monitorId: "monitor_123",
        network: { maxRetries: 3, baseDelayMs: 1 },
      });
    ...
    it("should fail-policy OPEN on exhaustion", async () => {
      vi.stubGlobal(
        "fetch",
        vi.fn().mockRejectedValue(new Error("Persistent outage")),
      );
      const guard = new ReplayGuard({
        apiKey: "sk_test_123",
        monitorId: "monitor_123",
        failPolicy: "OPEN",
        network: { maxRetries: 2, baseDelayMs: 1 },
      });
      const context = await guard.start("ext-id-1");
