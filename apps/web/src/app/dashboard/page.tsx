...
export default function DashboardPage() {
  ...
  const securityRisks = monitors?.filter((m: any) => m.status === 'DEGRADED' || (m.type === 'TUNNEL' && m.handshakeAge > 3600)).length ?? 0
    ...
    <div className="flex flex-col gap-12">
      ...
      <div className="grid gap-8 grid-cols-1 md:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard 
          ...
          subtitle={securityRisks > 0 ? 'Risk patterns detected' : 'Infrastructure safe'}
        />
      ...
      <section className="space-y-8 pt-4">
        <div className="flex items-center justify-between px-4">
          <h2 className="text-2xl font-black uppercase tracking-tight italic">Infrastructure Pulse</h2>
          <div className="flex items-center gap-4 bg-foreground/[0.03] px-6 py-2 rounded-full border border-border/5">
             <span className="text-[10px] text-muted-foreground font-black uppercase tracking-[0.2em]">Telemetry Active</span>
...
function StatCard({ 
  ...
  const colorClasses = {
    ...
    "destructive": "border-destructive/20 bg-destructive/[0.02] text-destructive",
  ...
  const content = (
    <Card className={`relative overflow-hidden glass-panel border border-border/10 rounded-[2.5rem] ${colorClasses} transition-all duration-500 hover:border-acid-lime/30 group hover:-translate-y-1`}>
      <CardHeader className="pb-4 pt-8 px-8 flex flex-row items-center justify-between">
        <CardTitle className="text-[10px] font-black uppercase tracking-[0.3em] opacity-80">{title}</CardTitle>
        <div className="opacity-40 group-hover:scale-110 transition-transform">{icon}</div>
      </CardHeader>
      <CardContent className="px-8 pb-8">
        <div className="text-5xl font-black tracking-tighter text-foreground">{value}</div>
        {progress !== undefined ? (
          <div className="flex items-center gap-4 mt-6">
             ...
             <span className="text-[10px] font-black opacity-80">{Math.round(progress)}%</span>
          </div>
        ) : (
          <p className="text-[10px] font-black mt-4 opacity-50 uppercase tracking-widest leading-relaxed">{subtitle}</p>
        )}
      </CardContent>
    </Card>
  );
