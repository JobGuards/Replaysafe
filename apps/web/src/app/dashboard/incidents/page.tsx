      ...
      <div className="bg-foreground/[0.02] p-12 rounded-[3rem] border border-border/5 relative overflow-hidden group">
        ...
        <div className="relative z-10 space-y-6">
          ...
          <p className="text-muted-foreground text-lg md:text-xl font-medium leading-relaxed max-w-2xl">
            ...
            latency spike, and missed heartbeat captured for deep forensic analysis.
          </p>
        </div>
      </div>
      ...
      {!isLoading && incidents && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          ...
          <StatusStat 
            ...
            isDestructive={incidents.filter((i: any) => !i.resolvedAt).length > 0}
          />
          <StatusStat 
            label="System Health" 
            value="99.98%" 
      ...
      <div className="flex flex-col gap-4 relative">
        ...
        {isLoading ? (
          <div className="space-y-6">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="h-32 rounded-[2rem] bg-foreground/[0.02] border border-border/5 animate-pulse" />
            ))}
          </div>
        ) : error ? (
          <div className="flex flex-col items-center justify-center py-24 text-destructive gap-4 glass-panel rounded-[2rem] border-destructive/10">
            ...
            <p className="font-black uppercase tracking-widest text-xs">Failed_to_Synchronize_Vault</p>
          </div>
...
function StatusStat({ label, value, icon, isDestructive = false }: any) {
  return (
    <div className={`p-8 rounded-[2rem] border border-border/5 bg-foreground/[0.01] flex flex-col gap-3 group hover:border-border/10 transition-all ${isDestructive ? 'hover:bg-destructive/[0.02]' : 'hover:bg-acid-lime/[0.02]'}`}>
      <div className="flex items-center justify-between">
        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground">{label}</span>
        {icon}
      </div>
      <div className={`text-4xl font-black italic tracking-tighter transition-colors ${isDestructive ? 'text-destructive group-hover:text-destructive' : 'text-foreground group-hover:text-acid-lime'}`}>
        {value}
      </div>
    </div>
  )
...
function IncidentCard({ incident, index }: any) {
  ...
  return (
    <div className={`relative flex items-start gap-10 group transition-all duration-500 animate-in fade-in slide-in-from-bottom-8 fill-mode-both`} style={{ animationDelay: `${index * 100}ms` }}>
      {/* Timeline Node */}
      <div className="hidden md:flex mt-12 w-16 h-16 rounded-full items-center justify-center shrink-0 z-10 relative">
        <div className={`absolute inset-0 rounded-full border-2 border-background transition-all duration-500 ${isResolved ? 'bg-emerald-500/10' : 'bg-destructive/10 animate-pulse'}`} />
        <div className={`w-3 h-3 rounded-full shadow-lg ${isResolved ? 'bg-emerald-500' : 'bg-destructive animate-ping'}`} />
      </div>

      {/* Card Content */}
      <div className={`flex-1 glass-panel p-8 md:p-10 rounded-[2.5rem] border border-border/5 hover:border-border/10 transition-all shadow-2xl hover:shadow-acid-lime/5 group/card`}>
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <IncidentTypeBadge type={incident.type} />
              <div className="h-[1px] w-6 bg-border/20" />
              <span className="text-[10px] font-mono text-muted-foreground/60">{incident.id.slice(0, 8)}</span>
            </div>
          ...
          <div className="flex items-center gap-6">
            <div className="text-right">
              <div className={`text-[10px] font-black uppercase tracking-[0.2em] mb-1 ${isResolved ? 'text-emerald-500' : 'text-destructive'}`}>
                {isResolved ? 'Duration' : 'Ongoing_Downtime'}
              </div>
              <div className={`text-2xl font-black italic tracking-tighter flex items-center gap-2 justify-end`}>
            ...
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${isResolved ? 'bg-emerald-500/5 text-emerald-500' : 'bg-destructive/5 text-destructive'}`}>
...
function IncidentTypeBadge({ type }: { type: string }) {
  ...
  const config = configs[type] || { icon: ShieldAlert, label: type, color: 'blue' }
  const Icon = config.icon

  return (
    <div className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest bg-foreground/[0.03] border border-border/5`}>
