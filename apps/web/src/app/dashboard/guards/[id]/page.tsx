        ...
        <div className="px-4 py-3 space-y-2 bg-background/20">
          {entries.map(([k, v]) => (
            <div key={k} className="flex items-center justify-between gap-4">
              ...
              <span className="text-[10px] font-mono text-acid-lime/80 truncate max-w-[200px]">
                {String(v)}
              </span>
            </div>
          ))}
...
function EffectTimelineNode({
  ...
  const typeIcon =
    effect.type === "HTTP" || effect.type === "WEBHOOK" ? (
  ...
  if (effect.startedAt && effect.finishedAt) {
    const ms =
      new Date(effect.finishedAt).getTime() -
      new Date(effect.startedAt).getTime();
  ...
  return (
    <div className="relative pl-14 group">
      {/* Timeline node dot */}
      <div
        className={`absolute left-0 top-1 w-10 h-10 rounded-xl flex items-center justify-center border z-10 transition-all group-hover:scale-110 ${
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <span
                className={`text-[8px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded border flex items-center gap-1.5 ${
                  cfg.color === "text-acid-lime"
                    ? "bg-acid-lime/10 text-acid-lime border-acid-lime/20"
                    : cfg.color === "text-blue-400"
          ...
          {effect.metadata && (
            <div className="p-4 rounded-xl bg-background/40 border border-border/5 space-y-2">
              ...
              <div className="text-[10px] font-medium text-muted-foreground truncate italic">
                {effect.metadata.message ||
                  JSON.stringify(effect.metadata).slice(0, 48) + "…"}
              </div>
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="relative space-y-6 before:absolute before:left-5 before:top-2 before:bottom-2 before:w-[1px] before:bg-border/10">
            {effects.length === 0 && (
              <p className="pl-14 text-muted-foreground/40 text-sm italic">
        ...
        <div className="space-y-10">
          <div className="glass-panel border border-border/5 rounded-[2.5rem] p-8 bg-foreground/[0.02] space-y-8">
            <h3 className="text-sm font-black uppercase tracking-widest italic text-muted-foreground">
              Session Intel
            </h3>
            ...
            <div className="space-y-6">
              ...
              {execution.workflowId && (
                <div className="flex items-start justify-between gap-4">
                  <span className="text-xs font-bold text-muted-foreground/60 flex-shrink-0">
                    Workflow
                  </span>
                  <span className="text-[10px] font-mono text-acid-lime/70 text-right truncate">
                    {execution.workflowId}
                  </span>
                </div>
              )}
              {execution.agentId && (
                <div className="flex items-start justify-between gap-4">
                  ...
                  <span className="text-[10px] font-mono text-muted-foreground/60 text-right truncate">
                    {execution.agentId}
                  </span>
                </div>
              )}
          ...
          {unknown > 0 && (
            <div className="glass-panel border border-orange-400/20 rounded-[2.5rem] p-8 bg-orange-400/5 space-y-6">
              ...
              <p className="text-xs text-orange-400/70 leading-relaxed font-medium">
                {unknown} side effect{unknown > 1 ? "s" : ""} timed out before a
                confirmed response was received. The operation may have
                completed on the provider side. Provider-side verification
                arrives in Phase 7.
              </p>
            </div>
          )}
          ...
          <div className="glass-panel border border-border/5 rounded-[2.5rem] p-8 bg-destructive/5 space-y-6">
            ...
            <p className="text-xs text-destructive/70 leading-relaxed font-medium">
              This session is protected by ReplayGuard. Side effects marked as
              &quot;Skipped&quot; were intercepted to prevent destructive
              duplication in this retry cycle.
            </p>
          </div>
        </div>
      </div>
