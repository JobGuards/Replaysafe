...
export default function Analytics() {
  ...
  const [selectedRange, setSelectedRange] =
    React.useState<(typeof timeRanges)[number]>("7d");
  const [activeTab, setActiveTab] = React.useState<
    "overview" | "intelligence" | "safety"
  >("overview");
  ...
  const paginatedPatterns = allPatterns.slice(
    (intelligencePage - 1) * itemsPerPage,
    intelligencePage * itemsPerPage,
  );

  const projectTrend = overview?.projectTrend || [];
  const safetyStats = overview?.safetyStats || {
  ...
  const stats = useMemo(() => {
    if (monitors.length === 0)
      return { uptime: 0, totalHeartbeats: 0, failureRate: 0 };

    const totalHealth = monitors.reduce(
      (acc: number, m: any) => acc + (m.healthScore || 0),
      0,
    );
    ...
    const totalHeartbeats = monitors.reduce((acc: number, m: any) => {
      ...
      return (
        acc +
        summary.reduce((sum: number, s: any) => sum + (s.totalCount || 0), 0)
      );
    }, 0);
    ...
    <div className="flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-[1400px] mx-auto pb-20">
      ...
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1 uppercase font-bold tracking-widest opacity-60">
            System Stress & Reliability Mapping
          </p>
        </div>
            ...
            <div className="glass-panel border border-border/10 rounded-3xl p-8 shadow-xl">
              ...
              <div className="h-[300px] w-full flex items-center justify-center">
                <div className="text-center">
                  <p className="text-sm text-muted-foreground uppercase font-black tracking-widest opacity-40">
                    System state is nominal
                  </p>
                  <p className="text-[10px] text-muted-foreground mt-2 italic">
                    Zero cascading failures detected in the current window
                  </p>
                </div>
              </div>
            </div>
                ...
                {totalIntelligencePages > 1 && (
                  <div className="flex items-center gap-2 bg-card/50 backdrop-blur rounded-lg border border-border/10 p-1">
                    <Button
                      ...
                      onClick={() =>
                        setIntelligencePage((p) => Math.max(1, p - 1))
                      }
                    >
                    ...
                    <Button
                      ...
                      onClick={() =>
                        setIntelligencePage((p) =>
                          Math.min(totalIntelligencePages, p + 1),
                        )
                      }
                    >
            ...
            <div className="flex flex-col gap-4 min-h-[300px]">
              {paginatedPatterns.length > 0 ? (
                paginatedPatterns.map((pattern: any, idx: number) => (
                  <div
                    key={idx}
                    className="glass-panel border border-border/10 rounded-2xl p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-6 animate-in fade-in slide-in-from-right-4 duration-300 hover:border-acid-lime/20 transition-colors"
                  >
                    <div className="flex-1 space-y-2">
                      <div className="flex items-center gap-3">
                        <span
                          className={`text-[9px] font-black uppercase tracking-[0.2em] px-2 py-0.5 rounded ${
                            pattern.type === "CASCADING"
                              ? "bg-purple-500/20 text-purple-400"
                              : pattern.type === "STREAK"
                                ? "bg-orange-500/20 text-orange-400"
                                : pattern.type === "SECRET_RISK"
                        ...
                        <span className="text-[10px] font-black text-muted-foreground/40 uppercase tracking-widest">
                          {new Date(
                            pattern.lastSeenAt || pattern.createdAt,
                          ).toLocaleString([], {
                            dateStyle: "medium",
                            timeStyle: "short",
                          })}
                    ...
                    <div className="flex-shrink-0 flex items-center gap-4">
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        className="rounded-lg text-[9px] font-black uppercase tracking-widest h-8 px-4 border-border/10"
                      >
                        Details
                      </Button>
                    </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 gap-4">
              {monitors.map((monitor: any) => (
                <div
                  key={monitor.id}
                  className="glass-panel border border-border/10 rounded-2xl p-6 shadow-xl flex flex-col lg:flex-row gap-6 items-center"
                >
                  ...
                  <div className="flex-shrink-0">
                    <Link href={`/dashboard/monitors/${monitor.id}`}>
                      <Button
                        className="rounded-xl font-black uppercase tracking-widest text-[9px] px-6 h-9"
                        variant="outline"
                      >
                        Inspect
                      </Button>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={deduplicationTrend}>
                  ...
                  <XAxis
                    dataKey="date"
                    tickFormatter={(val) =>
                      new Date(val).toLocaleDateString([], { weekday: "short" })
                    }
                    axisLine={false}
                    tickLine={false}
                    tick={{
