...
export default function PublicStatusPage() {
  ...
  const {
    ...
    error,
    isLoading,
  } = useSWR(`/api/public/status/${slug}`, () =>
    fetch(
      `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:4040"}/api/public/status/${slug}`,
    ).then((res) => {
      ...
      <div className="flex flex-col items-center justify-center min-h-screen bg-background gap-6">
        <div className="p-6 bg-destructive/10 rounded-3xl border border-destructive/20 text-center">
          ...
          <p className="text-muted-foreground mt-2">
            The status page you are looking for does not exist or is private.
          </p>
        </div>
      </div>
    );
      ...
      <div className="max-w-5xl mx-auto space-y-12">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-8 border-b border-border/10 pb-12">
          <div className="text-center sm:text-left">
            <div className="flex items-center gap-2 justify-center sm:justify-start mb-2">
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.2em] text-acid-lime">
                Sovereign Sentinel Active
              </span>
            </div>
          ...
          <div
            className={`px-8 py-6 rounded-[2rem] border-2 transition-all duration-500 shadow-2xl flex items-center gap-6 ${
              allOperational
                ? "bg-acid-lime/5 border-acid-lime/20 shadow-acid-lime/10"
                : "bg-destructive/5 border-destructive/20 shadow-destructive/10"
            }`}
          >
            <div
              className={`w-4 h-4 rounded-full animate-ping ${allOperational ? "bg-acid-lime" : "bg-destructive"}`}
            />
            <span
              className={`text-2xl font-black uppercase tracking-widest ${allOperational ? "text-acid-lime" : "text-destructive"}`}
            >
              {allOperational
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="glass-panel border border-border/10 rounded-[2.5rem] p-8 bg-acid-lime/[0.02] relative overflow-hidden group">
            ...
            <div className="text-[10px] font-black uppercase tracking-[0.3em] text-acid-lime mb-4">
              Retry Safety
            </div>
            <div className="text-5xl font-black text-foreground mb-2">
              {project.stats.retrySafety}%
            </div>
            <p className="text-xs text-muted-foreground font-medium">
              Replay-safe execution and side-effect deduplication across all
              autonomous retries.
            </p>
          </div>
          ...
          <div className="glass-panel border border-border/10 rounded-[2.5rem] p-8 bg-foreground/[0.02] relative overflow-hidden group">
            ...
            <div className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground/60 mb-4">
              Rollback Health
            </div>
            <div className="text-5xl font-black text-foreground mb-2">
              {project.stats.rollbackHealth}%
            </div>
            <p className="text-xs text-muted-foreground font-medium">
              Success rate of automated compensation hooks during job failures.
            </p>
          </div>
          ...
          <div className="glass-panel border border-border/10 rounded-[2.5rem] p-8 bg-foreground/[0.02] relative overflow-hidden group">
            ...
            <p className="text-xs text-muted-foreground font-medium">
              Independent monitoring nodes observing your infrastructure.
            </p>
          </div>
        </div>
        ...
        <div className="space-y-8">
          <h2 className="text-[10px] font-black uppercase tracking-[0.4em] text-muted-foreground/40 px-4 italic">
          ...
          <div className="grid grid-cols-1 gap-6">
            {project.monitors.map((monitor: any) => (
              <div
                key={monitor.id}
                className="glass-panel border border-border/10 rounded-[3rem] p-10 shadow-xl group hover:border-acid-lime/30 transition-all duration-500"
              >
                <div className="flex flex-col lg:flex-row items-center gap-12">
                  <div className="flex-shrink-0">
                    <HealthScoreBadge
                      ...
                      status={
                        monitor.healthScore > 90
                          ? "optimal"
                          : monitor.healthScore > 70
                            ? "warning"
                            : "critical"
                      }
                    />
                  ...
                  <div className="flex-1 w-full">
                    <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-8">
                      <div>
                        ...
                        {monitor.regionalStatus &&
                          Object.keys(monitor.regionalStatus).length > 0 && (
                            <div className="flex flex-wrap gap-3 mt-4">
                              {Object.entries(monitor.regionalStatus).map(
                                ([region, data]: [string, any]) => (
                                  <div
                                    key={region}
                                    className="flex items-center gap-2 px-3 py-1 bg-foreground/[0.03] rounded-lg border border-border/5 group/region"
                                  >
                                    <div
                                      className={`w-1.5 h-1.5 rounded-full ${data.status === "UP" ? "bg-acid-lime" : "bg-destructive"} ${data.status === "UP" ? "animate-pulse" : ""}`}
                                    />
                      ...
                      <div className="text-right hidden md:block">
                        <div className="text-[10px] font-black text-muted-foreground/40 uppercase tracking-widest mb-1 italic">
                          Last Heartbeat
                        </div>
                      ...
                      <div className="pt-8 border-t border-border/5 space-y-4">
                        ...
                        <div className="space-y-3">
                          {monitor.incidents.map((incident: any) => (
                            <div
                              key={incident.id}
                              className="flex items-center justify-between p-4 bg-foreground/[0.02] rounded-2xl border border-border/5 hover:bg-foreground/[0.04] transition-colors"
                            >
                              <div className="flex items-center gap-4">
                                <div
                                  className={`w-2 h-2 rounded-full ${incident.resolvedAt ? "bg-blue-500" : "bg-destructive animate-pulse"}`}
                                />
        ...
        <div className="pt-24 pb-12 flex flex-col items-center gap-6">
          ...
          <p className="text-muted-foreground text-sm font-medium text-center max-w-md">
            All monitors are cryptographically verified. Replaysafe provides{" "}
