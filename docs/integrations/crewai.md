...
The recommended pattern for Python-based CrewAI is to route all external tool calls through a lightweight TypeScript proxy service that uses StillUp:

```
CrewAI Agent → Tool → JS Proxy Service (StillUp) → External API
```
...
async function runCrewTool(
  ...
  return withReplayGuard(
    ...
    async (guard) => {
      ...
      return guard.crewai(toolName, inputs, () => dispatchTool(toolName, inputs));
    },
...
await runCrewTool(
  'web_search',
  { query: 'StillUp ReplayGuard pricing 2025', depth: 3 },
...
For pure Python setups, call the StillUp API directly inside your custom tool:
...
STILLUP_API_KEY = os.environ['STILLUP_API_KEY']
...
def guarded_tool_call(tool_name: str, inputs: dict, run_id: str, actual_fn):
    ...
    session = requests.post(f'{STILLUP_URL}/api/guards/session', 
        headers={'x-api-key': STILLUP_API_KEY},
    ...
    check = requests.post(f'{STILLUP_URL}/api/guards/verify',
        headers={'x-api-key': STILLUP_API_KEY},
    ...
    # Mark complete
    requests.patch(f'{STILLUP_URL}/api/guards/execution/{execution_id}',
        headers={'x-api-key': STILLUP_API_KEY},
        json={'status': 'SUCCESS'}
    )
