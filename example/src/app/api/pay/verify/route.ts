...
export async function POST(req: Request) {
  try {
    ...
    if (keySecret) {
      ...
      console.warn(
        "RAZORPAY_KEY_SECRET is not set. Skipping signature verification (simulation mode).",
      );
    }

    console.log(
      `[VERIFY] Initiating verify for order ${razorpay_order_id}, attempt #${attemptNumber}`,
    );
      ...
      async (guard) => {
        ...
        const fulfillmentResult = await guard.wrap(
          ...
          async () => {
            ...
            console.log(
              `[EXECUTE_SIDE_EFFECT] Fulfilling order ${razorpay_order_id} (Credits Provisioning)`,
            );
            ...
            return {
              status: "COMPLETED",
              creditsAdded:
                planId === "basic" ? 100 : planId === "premium" ? 500 : 3000,
        ...
        if (simulateCrash && attemptNumber === 1) {
          console.log(
            "[SIMULATED_CRASH] Threw simulated crash post-fulfillment!",
          );
